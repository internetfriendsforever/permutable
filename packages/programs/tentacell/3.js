    ...
    return values => {
      ...
      for (let col = 0; col < cols; col++) {
        for (let row = 0; row < rows; row++) {
          ...
          const angle = simplex.noise3D(col / noiseSize, row / noiseSize, time / (1000 / 0.3)) * Math.PI * 2
          const angle2 = Math.pow(simplex.noise3D(col / noiseSize / 5, row / noiseSize / 5, time / (5000 / 0.3)) * Math.PI * 2, 10)
          ...
          tentacle.forEach((current, i) => {
            ...
            if (!prev) {
              ...
              current.x += ((prev.x + Math.cos(a) * 20) - current.x) * 1
              current.y += ((prev.y + Math.sin(a) * 20) - current.y) * 1
            }

            context.strokeRect(current.x, current.y, 1, 1)
          })
