    ...
    return values => {
      ...
      for (let col = 0; col < cols; col++) {
        for (let row = 0; row < rows; row++) {
          ...
          const angle = simplex.noise3D(col / noiseSize, row / noiseSize, time / 2000) * Math.PI * 2
          const angle2 = Math.pow(simplex.noise3D(col / noiseSize / 5, row / noiseSize / 5, time / 6000) * Math.PI * 2, 4)

          context.strokeStyle = 'rgba(255, 255, 255, ' + angle2 + ')'
          ...
          tentacle.forEach((current, i) => {
            ...
            if (!prev) {
              ...
              current.x += ((prev.x + Math.cos(a) * Math.random() * 30) - current.x) * 1
              current.y += ((prev.y + Math.sin(a) * Math.random() * 30) - current.y) * 1
            }
